    ...
    <>
              ...
              <div className="size-10 relative cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                  <div className={twMerge(
                    "w-5 h-0.5 bg-zinc-300 -translate-y-1 transition duration-500 ease-in-out",
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="absolute inset-2 rounded-md md:rounded-lg lg:rounded-xl bg-zinc-800 mt-24 md:mt-28 z-0 overflow-hidden">
              <div className="absolute top-1/2 left-full -translate-y-1/2 -translate-x-1/2 -z-10">
                <Hexagon className="size-[700px]" size={700} />
              </div>

              <div className="absolute top-1/2 left-full -translate-y-1/2 -translate-x-1/2 -z-10">
                <Hexagon className="size-[1100px]" size={1100} reverse duration={60} />
              </div>
                ...
                <nav className="flex flex-col items-center gap-12 md:gap-16">
                  {navLinks.map(({ title, href }, index) => (
                    <motion.a
                      ...
                      key={title}
                    >
                      <span className="text-4xl md:text-5xl font-heading font-black text-zinc-500 hover:text-zinc-300 transition duration-300">
                        {title}
                      </span>
                    </motion.a>
                  ))}
